...
(* Re-export viewport types for local use *)
type segment = Viewport.segment = { start_gi : int; end_gi : int }
let wrap_line = Viewport.wrap_line
let find_segment_index = Viewport.find_segment_index
let cursor_col_in_segment = Viewport.cursor_col_in_segment
let grapheme_at_col_in_segment = Viewport.grapheme_at_col_in_segment
let iter_clusters = Viewport.iter_clusters
